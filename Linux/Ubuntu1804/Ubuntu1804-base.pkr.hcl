...
variable "iso_file_checksum" {
  type    = string
  default = "file:images/ubuntu1804.iso.shasum"
  description = "The checksum for the Ubuntu ISO"
}

variable "iso_url" {
  type    = string
  default = "images/ubuntu1804.iso"
...
source "virtualbox-iso" "ubuntu1804-iso" {
  ...
  boot_command = [
    ...
    "set gfxpayload=keep",
    "<enter>",
    "linux /install/vmlinuz preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/bootstrap/preseed.cfg auto-install/enable=true fb=false debconf/frontend=noninteractive debconf/priority=critical config_server=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ net.ifnames=0 ---",
    "<enter>",
